...
public class NoteLister {
	...
	private void listNotes() throws Exception {
		...
		for (Notebook nbook : notebooks) {
			...
			for (int i = 0; i < noteCount; i += 50) {
				...
				for (Note note : notes) {
					this.cnt++;
					int tgcnt = note.getTagNamesSize();
					if (tgcnt == 0) {
						System.out.println(" " + cnt + " " + note.getTitle()
								+ " - tags: " + tgcnt);
					}
				}
			}
	...
	public Map readProperties() {
		...
		String token = "nothing found";
		String notebookname = "default_notebook";

		while (enumeration.hasMoreElements()) {
			String var = (String) enumeration.nextElement();
			String val = properties.getProperty(var);
			if (var.equalsIgnoreCase("developertoken")) {
				token = val;
			}

			if (var.equalsIgnoreCase("notebookname")) {
				notebookname = val;
			}
		}

		propmap.put("token", token);
		propmap.put("notebookname", notebookname);
		return propmap;
	}
